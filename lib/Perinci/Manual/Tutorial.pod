...
just because you switch language, not having to learn different API's for each
web service. Another philosophy is to focus on functions (as opposed to classes
or objects, or other kind of code entity). Function is the basic unit of reuse
of a program. By making functions more powerful, reusable, flexible, we can
greatly improve the development experience.


=head1 GETTING STARTED

Let's start this tutorial by writing an example function:
...
the above example, it's even longer than the actual "business" routine.
...
Let's write our first function metadata.
...
contain quite a few items (some seem weird and needless). Let's go through them
...
This property is always needed because there used to be an incompatible, first
version of the metadata format (called 1.0). Specifications evolve, fact of
life. To keep backward compatibility, if C<v> is not specified, it is assumed to
be 1.0.
...
Here you can notice one thing: there's no longer any need to declare @ISA (a
quirkiness of Exporter) and @EXPORT_OK. All functions which have metadata are
assumed to be exportable.

Now let's use our function again:
...
regularly. The other properties work though.)


=head1 COMMAND-LINE PROGRAMS

Suppose you want to create a CLI program for your module. Some basic
functionality of a CLI program include command-line options processing and
...
another tool, we can replace all that work with just a single line of code.
Please install L<Perinci::CmdLine> first if you don't have it on your system.

But first, let's add some text to our metadata:
...
(taken from function arguments), help/usage message (using summary and other
information from metadata), output result in a variety of formats (YAML, JSON,
text, and more), among other things. Other features not demonstrated in this
tutorial include subcommands, logging, Bash tab completion.
...
(and other tools) can provide command-line interface (and do other stuffs) for
remote code entities.

=item * Parsing argument value

Since the a1 and a2 arguments are not simple string/scalar, Perinci::CmdLine
tries to interpret the command-line options as JSON or YAML.

=item * Markdown

The C<description> property is in Markdown format.
...
in metadata can also be used to generated POD documentation. Combined with tools
...
but also to access metadata:
 ...
 [200,"OK",["pl:/MyApp/add_array"]]

A host is viewed as a tree of code entities with / at the root. By using the
Riap action C<list> to list contents of package entities, and by using the Riap
action C<meta> to view an entity's metadata, we have a self-documenting and
discoverable service. This is applicable to every web service implementing the
Riap protocol.
...
are naked. Let's see how this works in CLI and over HTTP:
...
What's covered in this tutorial is just the tip of the iceberg. Metadata can be
